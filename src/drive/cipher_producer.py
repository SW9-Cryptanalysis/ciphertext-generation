...
class CipherProducer(mp.Process):
	...
	def __init__(self, queue: MPQueue[Any], start_and_total: tuple[int, int], *args, **kwargs):
	...
	def run(self):
		...
		for i in range(self.start_idx, self.start_idx + self.total_to_generate):
			try:
				...
				filename = f"c_{len(cipher.plaintext)}_{cipher.difficulty}_{i}_{os.getpid()}.json"
	...
	def generate_cipher(self) -> SubstitutionCipher:
		...
		sliced_text = fetcher.get_random_book_slice(book_text, self.MIN_LEN, self.MAX_LEN)
