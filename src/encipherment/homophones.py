...
def extract_homophones(cipher_symbols: int, frequencies: dict) -> dict[str, int]:
    """Extract a dictionary mapping each letter to its number of homophones.

    Result is based on letter frequencies in English text.

    Args:
        cipher_symbols (int): Total number of cipher symbols to distribute among letters
        frequencies (dict): A dictionary mapping each letter to its frequency.
    ...
    ideal_homophones = [
        max(Decimal("1"), count) if count > 0 else Decimal("0")
        for count in ideal_homophones
    ]

    homophones_dict: dict[str, int] = dict(
        zip(frequencies.keys(), map(round, ideal_homophones), strict=False),
    )
...
def get_homophones(homophones: list[int], letter_occurrences: int) -> list[int]:
    """Get a list of homophones for a letter from the list of available homophones.

    Ensures that all homophones are used at least once if letter_occurrences
    is greater than the number of available homophones.
    ...
    if letter_occurrences < len(homophones):
        return random.sample(
            homophones,
            letter_occurrences,
        )  # Impossible to use all homophones, so just sample
