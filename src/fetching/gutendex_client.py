...
class GutendexClient:
    ...
    def __init__(self, batch_size: int = 35, timeout: int = 15, sleep_time: float = 1.0, logger: logging.Logger | None = None) -> None:
    ...
    def fetch_raw_bookshelves(self, book_ids: list[str]) -> dict[str, list[str]]:
        ...
        self.logger.info(f"Starting batch extraction ({total_batches} total network requests needed)...")
        ...
        for i in range(0, len(book_ids), self.batch_size):
            ...
            try:
                ...
                self.logger.warning("Skipping this batch and continuing to the next one...")
                
            if current_batch_num % 50 == 0 or current_batch_num == total_batches:
                self.logger.info(f"Processed batch {current_batch_num}/{total_batches}...")
