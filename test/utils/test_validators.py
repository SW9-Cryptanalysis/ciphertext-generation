...
from utils.validators import in_range, strongly_typed_optional, lower_case_no_spaces_alpha_string, is_alpha_lowercase_no_spaces
...
class TestStronglyTypedOptionalValidator:
	...
	def test_illegal_type_value(self):
		...
		assert "Parameter `optional_param` must be of type int or None." in str(excinfo.value)
	...
	def test_value_of_unexpected_union_type(self):
		...
		assert "Parameter `optional_param` must be of type int, str, or None." in str(excinfo.value)

class TestLowerCaseNoSpacesAlphaStringValidator:
	def test_valid_string(self):
		lower_case_no_spaces_alpha_string("validstring", name="test_param", type_hint=str)

	def test_string_with_spaces(self):
		with pytest.raises(ValueError) as excinfo:
			lower_case_no_spaces_alpha_string("invalid string", name="test_param", type_hint=str)
		assert "Parameter `test_param` must be a lowercase alphabetic string with no spaces." in str(excinfo.value)

	def test_string_with_uppercase(self):
		with pytest.raises(ValueError) as excinfo:
			lower_case_no_spaces_alpha_string("InvalidString", name="test_param", type_hint=str)
		assert "Parameter `test_param` must be a lowercase alphabetic string with no spaces." in str(excinfo.value)

	def test_string_with_non_alpha(self):
		with pytest.raises(ValueError) as excinfo:
			lower_case_no_spaces_alpha_string("invalid123", name="test_param", type_hint=str)
		assert "Parameter `test_param` must be a lowercase alphabetic string with no spaces." in str(excinfo.value)
